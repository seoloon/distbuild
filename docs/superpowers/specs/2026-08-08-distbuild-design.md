# DistBuild — Design Spec

Date: 2026-08-08
Status: Approved (author-supplied spec, executed via `claude/PROMPT.md`)
...
All message types derive `Serialize, Deserialize`, tagged with `#[serde(tag = "type")]` inside one `Message` enum.
...
Zip → `<worker>/jobs/<job_id>/artifacts.distbuild.zip`, streamed over WebSocket in binary frames with a small header `{ job_id, chunk_index, total_chunks }`.
...
- `<LogViewer />` — virtualized, ANSI color support (`ansi_up`), auto-scroll toggle
...
5. **Type-safe protocol.** JS side uses generated TS types (`ts-rs` or `specta`) from Rust definitions — no hand-written duplicates.
...
Work proceeds phase by phase. After Phase 0, the workspace `Cargo.toml`, folder tree, `justfile`, and initial `apps/desktop/src-tauri/Cargo.toml` are presented for review; implementation pauses for explicit "go" before Phase 1 begins. The same phase-by-phase confirmation applies going forward unless the user says otherwise.
